...
void DebugScene::Initialize(Engine& engine) {
	...
	auto bossModel = load.LoadResource<LoadModel>("Res/Model/Enemy/Boss/Boss.mv1");
	...
	load.SetOnComplete(
		[stageModel, stageModel2, player, playerModel, enemy, enemyModel, stageBoneData, treasure, treasureModel1, treasureModel2, stair, exit]() {
			...
			std::unordered_map<int, Vector3> treasureSpawnPos = StageManager::GetInstance().GetTreasureSpawnPos(0);
